...
Help()
{
   ...
   echo "e     Path to RHEL ISO. It will Embedd the kickstart in a new ISO so you don't need to modify kernel args during boot(default=disabled)"
   echo
...
export GPG_TTY=$(tty)
echo "redhat" | gpg -b --passphrase-fd 0 tmp/boot-server/var/www/html/disk.img.xz
...
sed -i 's/quiet/inst.ks=hd:LABEL=RHEL-Server.x86_64:\/ks.cfg/g' isolinux/isolinux.cfg
sed -i 's/timeout 600/timeout 1/g' isolinux/isolinux.cfg
sed -i 's/RHEL-.-.-0-BaseOS-x86_64/RHEL-Server.x86_64/g' isolinux/isolinux.cfg

RHEL-8-5-0-BaseOS-x86_64

sed -i 's/quiet/inst.ks=hd:LABEL=RHEL-Server.x86_64:\/ks.cfg/g' EFI/BOOT/grub.cfg
sed -i 's/RHEL-.-.-0-BaseOS-x86_64/RHEL-Server.x86_64/g' EFI/BOOT/grub.cfg

rm -rf ../../images/rhel-custom.iso

mkisofs -o ../../images/rhel-custom.iso -b isolinux/isolinux.bin -c isolinux/boot.cat --no-emul-boot --boot-load-size 4 --boot-info-table -J -R -V  "RHEL-Server.x86_64" .

isohybrid --uefi ../../images/rhel-custom.iso

implantisomd5 ../../images/rhel-custom.iso
